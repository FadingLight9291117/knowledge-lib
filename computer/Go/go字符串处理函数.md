...
> go语言中处理string的函数，主要在strings和strconv这两个包中。

## strings 和 strconv 包

作为一种基本数据结构，每种语言都有一些对于字符串的预定义处理函数。Go 中使用 `strings` 包来完成对字符串的主要操作。

### 前缀和后缀(strings.HasPrefix&HasSuffix)
...
### 判断子字符串或字符在父字符串中出现的位置(索引)(strings.Index&LastIndex&IndexRune)
...
### 修改字符串大小写(strings.ToLower&ToUpper)
...
### 修剪字符串(strings.TrimSpace&Trim)

你可以使用 `strings.TrimSpace(s)` 来剔除字符串开头和结尾的空白符号；如果你想要剔除指定字符，则可以使用 `strings.Trim(s, "cut")` 来将开头和结尾的 `cut` 去除掉。该函数的第二个参数可以包含任何字符，如果你只想剔除开头或者结尾的字符串，则可以使用 `TrimLeft` 或者 `TrimRight` 来实现。

### 分割字符串(strings.Fields&Split)
...
### 从字符串中读取内容(strings.NewReader&ReadByte&ReadRune)
